...
test('Deve criar um pedido com 3 itens', function () {
	...
	order.addItem(new Item('1', 'Camiseta', 100, 10, 10, 10), 1);
	order.addItem(new Item('2', 'Calça', 50, 10, 10, 10), 3);
	order.addItem(new Item('3', 'Meia', 20, 10, 10, 10), 2);
...
test('A quantidade de itens no pedido não pode ser menor ou igual a zero', function () {
	const cpf = new Cpf('835.179.250-04');
	const order = new Order(cpf);
	expect(() => order.addItem(new Item('1', 'Camiseta', 100, 10, 10, 10), -1)).toThrow(
		new Error('Invalid item quantity.')
	);
	expect(() => order.addItem(new Item('1', 'Camiseta', 100, 10, 10, 10), 0)).toThrow(
		new Error('Invalid item quantity.')
	);
});
...
test('Não pode adicionar o mesmo item duas vezes em um pedido', function () {
	...
	const order = new Order(cpf);
	order.addItem(new Item('1', 'Camiseta', 100, 10, 10, 10), 1);
	expect(() => order.addItem(new Item('1', 'Camiseta', 100, 10, 10, 10), 2)).toThrow(
